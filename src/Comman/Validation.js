const Validation = (data, type) => {
    ...
    const { firstName, lastName, email, password, ConfirmPassword, fullName, Address, mobile, pincode, city, role, otp } = data
    ...
    if (type === "register") {
        ...
        if (role === "0") {
            err.push({ key: "role", message: "Please Select One Option" })
        }
        ...
        if (!otp) {
            err.push({ key: "otp", message: "Please Enter Your Otp" })
        } else if (otp.length < 6) {
            err.push({ key: "otp", message: "6 Digits Is Required" })
        }
    }
    else {
        // For Email
